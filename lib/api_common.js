 ...
 */
var API = function (componentAppid, componentAppSecret, authorizerAppid, authorizerRefreshToken, componentVerifyTicket, getComponentToken, saveComponentToken, getToken, saveToken) {
...
API.prototype.getComponentAccessToken = function * () {
  ...
  const data = yield this.request(url, options)
  // 过期时间，因网络延迟等，将实际过期时间提前10秒，以防止临界点
  const expireTime = new Date().getTime() + (data.expires_in - 10) * 1000
  const componentToken = ComponentAccessToken(data.component_access_token, expireTime)
