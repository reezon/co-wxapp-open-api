...
exports.assignPackage = function * (orderId, skuId, receiverAppId, requestId){
    var token = yield this.ensureAccessToken()
    var url = this.wxappPrefix + 'service-provider/package/assign?access_token=' + token.accessToken
    var data = {
        orderId, 
        skuId,
        receiverAppId,
...
exports.queryOrderList = function * (spuId, offset, limit){
    var token = yield this.ensureAccessToken()
    var url = this.wxappPrefix + 'service-provider/order/query_list?access_token=' + token.accessToken
    var data = {
        spuId, 
        offset,
        limit
